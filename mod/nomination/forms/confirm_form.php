...
class mod_nomination_confirm_form extends moodleform {
    ...
    public function definition() {
        ...
        $this->add_action_buttons(true, get_string('vote', 'mod_nomination'));
    }
    ...
    public function validation($data, $files) {
        ...
        foreach ($data as $field => $value) {
            if ((substr($field, 0, 9) == 'candidate') && $value >= 1) {
                if (in_array($value, $usedpositions)) {
                    $errors['header'] = get_string('posistioncanonlybeusedonce', 'mod_nomination');
                }
                $usedpositions[] = $value;
                if ($value == 1) {
                    $hasnumberone = true;
        ...
        if (!$hasnumberone) {
            $errors['header'] = get_string('musthavenumberone', 'mod_nomination');
        }
        ...
        if (empty($errors)) {
            for ($i = 0; $i < (count($usedpositions) - 1); $i++) {
                if ($usedpositions[$i] != ($usedpositions[$i + 1] - 1)) {
                    ...
                    $errors['header'] = get_string('nosequentialnumbering', 'mod_nomination');
                }
            }
        }
